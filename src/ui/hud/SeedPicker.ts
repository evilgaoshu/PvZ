...
export class SeedPicker extends Phaser.GameObjects.Container {
  ...
  constructor(scene: Phaser.Scene, allAvailablePlants: PlantCardConfig[], onConfirm: (plants: PlantCardConfig[]) => void) {
    super(scene, 400, 300);
    ...
    const title = scene.add.text(0, -190, '选择你的植物', { fontSize: '28px', color: '#fcd34d', fontStyle: 'bold' }).setOrigin(0.5);
    this.add(title);
    ...
    allAvailablePlants.forEach((plant, index) => {
      ...
      const card = new PlantCard(scene, -240 + col * 80, -80 + row * 100, plant, () => this.toggleSelection(plant));
      this.add(card);
    ...
    // 3. 确认按钮
    const confirmBtn = new GameButton(scene, 0, 260, { 
      text: '开始战斗！', 
      width: 180, 
