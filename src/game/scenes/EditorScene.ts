...
export class EditorScene extends BaseScene {
  ...
  protected onCreate(): void {
    ...
    // 提示信息
    this.add.text(400, 20, '关卡编辑器：点击网格最右侧添加僵尸', {
      fontSize: '18px',
      color: '#ffffff',
  ...
  private createGrid(): void {
    ...
    for (let row = 0; row < ROWS; row++) {
      ...
      const area = this.add.rectangle(x + CELL_WIDTH/2, y + CELL_HEIGHT/2, CELL_WIDTH, CELL_HEIGHT, 0x4ade80, 0);
  ...
  private createZombieSelector(): void {
    ...
    types.forEach((type, index) => {
      const btn = new GameButton(this, 80, 150 + index * 60, {
        text: type.label,
        width: 120,
        height: 45,
        backgroundColor: this.selectedZombieType === type.id ? type.color : 0x1e293b,
        fontSize: '16px'
  ...
  private createTimeline(): void {
    ...
    this.add.text(x, y - 30, '时间轴 (0s - 300s)', { fontSize: '14px', color: '#94a3b8' });
    ...
    // 拖拽层
    const dragArea = this.add.rectangle(x + width/2, y, width + 40, 60, 0xffffff, 0);
    dragArea.setInteractive({ draggable: true });
    
    this.timeLabel = this.add.text(x + width + 20, y, '0s', { 
      fontSize: '20px', 
      color: '#4ade80',
      fontStyle: 'bold'
    }).setOrigin(0, 0.5);
  ...
  private addZombieAt(row: number): void {
    ...
    const txt = this.add.text(0, 0, this.currentTime.toString(), { fontSize: '10px', color: '#fff' }).setOrigin(0.5);
    const typeTxt = this.add.text(0, 20, this.selectedZombieType.charAt(0), { fontSize: '10px', color: '#fcd34d' }).setOrigin(0.5);
    ...
    console.log(`Added ${this.selectedZombieType} at row ${row}, time ${this.currentTime}s`);
  }
  ...
  private exportLevel(): void {
    ...
    console.log("%c--- 导出的关卡配置 (YAML) ---", "color: #4ade80; font-weight: bold;");
    console.log(yamlStr);
    
    // 尝试复制到剪贴板
    navigator.clipboard.writeText(yamlStr).then(() => {
      alert('关卡配置已导出并复制到剪贴板！\n请粘贴到 data/levels/ 目录下的 yaml 文件中。');
    }).catch(() => {
